...
def test_cli_audit_skip_file(capsys):
    ...
    with patch("py_gradeup.sdk.PyGradeup.audit") as mock_audit, patch(
        ...
        "py_gradeup.core._should_modify", return_value=False
    ):
        ...
        mock_audit.return_value = AuditResult(
            ...
            None,
            [],
            {"reqs.txt": {"requests": "requests>=2.0.0"}},
            [],
            [],
            [],
            [],
...
def test_cli_security_none_details(capsys):
    """Test function."""
    with patch("py_gradeup.sdk.PyGradeup.security") as mock_sec, patch(
        "py_gradeup.core._get_target_files"
    ) as mock_get_target_files, patch("os.path.exists", return_value=True), patch(
        "py_gradeup.security._parse_dependencies", return_value={"pkg==1.0": "1.0"}
    ):
        mock_get_target_files.return_value = ["reqs.txt"]
        mock_sec.return_value = SecurityResult(
            True, {"pkg==1.0": [{"id": "CVE-1", "details": ""}]}
        )
...
def test_cli_graph_empty_tree_not_none(capsys):
    """Test function."""
    with patch("py_gradeup.sdk.PyGradeup.graph") as mock_graph, patch(
        "py_gradeup.core._get_target_files"
    ) as mock_get_target_files:
        mock_get_target_files.return_value = ["reqs.txt"]
...
def test_sdk_audit_skip_file(tmp_path):
    ...
    with patch("py_gradeup.sdk._should_modify", return_value=False), patch(
        "py_gradeup.sdk._get_target_files", return_value=[str(tmp_path / "reqs.txt")]
    ), patch("py_gradeup.sdk._get_py_files", return_value=[]):
...
def test_sdk_fix_commit_no_version_change(tmp_path):
    ...
    with patch("py_gradeup.sdk._get_current_python_version", return_value="3.8"), patch(
        "py_gradeup.sdk._find_target_python", return_value=("3.8", {})
    ), patch("py_gradeup.sdk._get_target_files", return_value=[]), patch(
...
def test_sdk_find_target_python_cleanup(tmp_path):
    ...
    pg = PyGradeup(str(tmp_path))
    with patch("py_gradeup.sdk._get_current_python_version", return_value="3.8"), patch(
        "py_gradeup.core._prepare_compile_targets", return_value=["/does/not/exist/999"]
    ):
        (tmp_path / "fake.txt").touch()
        with patch(
            "py_gradeup.sdk._get_target_files",
            return_value=[str(tmp_path / "fake.txt")],
        ):
            pg.audit()
...
def test_core_update_bounds_dry_run(tmp_path):
    ...
    assert (
        core._update_python_version_bounds(str(tmp_path), "3.9", dry_run=True) is True
    )
...
def test_core_find_target_python_missing_name(tmp_path):
    ...
    with patch("subprocess.run", side_effect=side_effect), patch(
        "py_gradeup.core._prepare_compile_targets", return_value=["fake.in"]
    ):
...
def test_core_backup_old_requirements_missing_name(tmp_path):
    ...
    mock_result.returncode = 0
    mock_result.stdout = json.dumps(
        {"install": [{"metadata": {"name": "pkg1"}}, {"metadata": {"version": "1.0"}}]}
    )
...
def test_core_find_target_python_cleanup_not_exists(tmp_path):
    ...
    with patch("subprocess.run", return_value=mock_result), patch(
        "py_gradeup.core._prepare_compile_targets", side_effect=mock_prepare
    ):
        core._find_target_python(["fake.txt"], "3.8")
